...
| ASI09 | Improper Error Handling | Medium | `PostToolUseFailure` hook pattern-matches errors and suggests recovery. Stack-specific `failure-patterns/` directories define known error patterns. | `.claude/hooks/post-failure.py`, `stacks/*/failure-patterns/` |
